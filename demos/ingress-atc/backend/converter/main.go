...
func V1ToV2(source v1.Backend) v2.Backend {
	return v2.Backend{
		...
		Spec: v2.BackendSpec{
			...
			Command:    source.Spec.Command,
			Replicas:   source.Spec.Replicas,
			PathPrefix: source.Spec.PathPrefix,
			Env: func() map[string]v2.Envvar {
				if source.Spec.Env == nil {
					return nil
				}
				result := map[string]v2.Envvar{}
				for name, value := range source.Spec.Env {
					result[name] = v2.Envvar{PlainText: value}
				}
				return result
			}(),
		},
	...
	return v1.Backend{
		...
		Spec: v1.BackendSpec{
			...
			Env: func() map[string]string {
				...
				for name, env := range source.Spec.Env {
					if env.PlainText == "" {
						continue
					}
					result[name] = env.PlainText
				}
				return result
			}(),
		},
